  ...
  return (
    <motion.div className="bg-white rounded-xl shadow-lg p-6 mt-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="text-center mb-8">
        <h3 className="text-3xl md:text-4xl font-extrabold mb-2 uppercase tracking-wide">CẢM NHẬN CỦA CÁC THÀNH VIÊN &amp; GIA ĐÌNH</h3>
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-medium">
                    {[0,1,2,3,4].map((i) => (
                      <span key={i} className={i < Math.round(review.rating) ? 'text-green-600' : 'text-gray-300'}>★</span>
                    ))}
                  </span>
                ...
                <div className="text-gray-800 text-base leading-relaxed mb-2">
                  {review.comment.length > 140 && !expandedComments[review.id] ? (
                    <>
                      ...
                      <span className="font-semibold underline cursor-pointer ml-1" onClick={() => setExpandedComments((p) => ({ ...p, [review.id]: true }))}>Hiện thêm</span>
                    </>
                  ) : review.comment.length > 140 && expandedComments[review.id] ? (
                    <>
                      {review.comment}
                      <span className="font-semibold underline cursor-pointer ml-1" onClick={() => setExpandedComments((p) => ({ ...p, [review.id]: false }))}>Ẩn bớt</span>
                    </>
                  ) : (
                    review.comment
                  )}
